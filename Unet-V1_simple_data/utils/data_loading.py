...
# removed mask_suffix parameter; removed scanning of masks for unique classes; removed treatment of data with only one channel; mask values returned as float 32; added overfitting dataset;
...
class OverfitDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0):
        ...
        if isfile(join(images_dir, listdir(images_dir)[dat])) and not listdir(images_dir)[dat].startswith('.'):
            self.ids = [splitext(listdir(images_dir)[dat])[0]] * len_data
        else:                                                                                                                                                                                                                                       
            raise RuntimeError(f'No input file found in {images_dir} at the given index, make sure you put your images there')                                                                                                                                                     
